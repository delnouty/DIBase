...
# 6. Allow clients to access their data
def get_client_data(client_id):
    cursor.execute('''
        SELECT Client_ID, Nom, Prenom, Email, Consentement_Marketing
        FROM Clients
        WHERE Client_ID = ?
    ''', (client_id,))
...
# 7. Allow clients to delete their data
...
# 8. Log data access for auditing purposes
def log_data_access(client_id, action):
    cursor.execute('''
        INSERT INTO Data_Access_Log (Client_ID, Action, Timestamp)
        VALUES (?, ?, ?)
    ''', (client_id, action, datetime.now()))
    conn.commit()
...
# 6. Access client data (e.g., Client_ID = 2)
client_data = get_client_data(2)
print("\nClient data for ID 2:", client_data)

# 7. Delete client data (e.g., Client_ID = 2)
delete_client_data(2)
print("\nClient data for ID 2 deleted.")

# 8. Log data access
log_data_access(2, 'Accessed client data')
